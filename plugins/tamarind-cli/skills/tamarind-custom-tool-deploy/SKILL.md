...
def drain_logs(version):
    ...
    while True:
        ...
        for event in page.items:
            print(event.message)
        # ONLY a null cursor ends the stream. Measured against a build that had
        # already gone terminal: the first page carried 288 lines, the second
        # carried ZERO with a *different* non-null cursor, and only the third
        # returned null. So an empty page is not the end, and neither emptiness
        # nor a changed cursor is a stop condition.
        if page.next_cursor is None:
            return
        # Check the deadline on EVERY pass, not just when the cursor repeats: a
        # stream that keeps handing back new empty cursors would otherwise spin
        # without ever consulting it. Sleep whenever a page brought nothing, so
        # waiting for the tail of a long build costs one request per interval.
...
Build-log paging has one stop condition: `next_cursor is None`. An empty page is NOT the end - measured, a terminal build served 288 lines, then an empty page under a NEW cursor, then null. Never stop on an empty page or on a cursor that stopped changing.
