...
`maxRuntimeSeconds` - and neither mints a version on its own. Those fields are re-read from
`config.json` on every publish, so `config.json` is their source of truth and editing it is how you
change them.

`displayName`, `description`, `functions` and `tags` are NOT in that set on the archive-upload path:
they are written when the tool record is CREATED and are not re-applied from a later `config.json`.
Set them correctly on the first deploy, or change them in the web UI afterwards.
