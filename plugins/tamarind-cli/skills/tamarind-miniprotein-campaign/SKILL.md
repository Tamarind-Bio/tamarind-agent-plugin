...
- resolve every tool and setting live with `tamarind-tool-discovery`;
...
Research the target's biology first, then write the plan to a file on disk and restate its frozen values in the conversation. It records:
...
Run one small canary per generation method **on the real campaign target with production settings**, and give each method exactly one verdict:
...
- ~20,000 designs before gating, fanned out with `tamarind-batch`, never a shell loop over `submit`.
...
[references/instrument.md](references/instrument.md) holds the arms, the score algebra, the seed tiers and the funnel. Load-bearing invariants:
