...
**`validateOnly` checks the archive, not the values inside it.** It confirms the files are present and that `config.json` parses; it does NOT check that `memory`, `cpu` or `gpuType` hold values the platform accepts. Measured on staging: `memory` of `2Gi`, `4Gi` and `16Gi` each came back `valid: true` with no warnings and were then refused by the real deploy as `config.json is invalid` - an error naming no field - while `8Gi` deployed. So a deploy that 400s on a config that just validated is almost always a resource value; copy `memory`/`cpu`/`gpuType` from a tool that already builds rather than guessing.

**A deploy rejected that way has already taken the name.** It creates the tool record first, so the name is now held by an empty `Draft` (`hasSource: false`, no version). Retrying with `expectNew=True` after you fix the config then fails `custom_tool_name_taken`, and the hint blames another member - but it was this call, moments earlier. Drop `expectNew` for that retry (verified: the corrected retry returns `build`), or have the user delete the empty Draft first. Do NOT read that 409 as evidence someone else claimed the name unless you saw the tool exist before your first attempt.
