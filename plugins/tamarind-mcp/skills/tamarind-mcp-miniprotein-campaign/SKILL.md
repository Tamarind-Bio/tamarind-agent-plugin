...
**`fold_class` needs `biotite`, and without it the fold-diversity evidence is silently absent.** The classifier resolves secondary structure from DSSP codes, a bare SS string, HELIX/SHEET records in the PDB, or `biotite.annotate_sse` — in that order. Measured on five real designs from three methods: generator PDBs carry **no HELIX/SHEET records at all**, so on a machine without biotite every one came back `fold_class = NOT_RUN`. Install biotite with numpy. With it, those same five structures classified as `all_alpha` — which is the campaign's own answer to the >=10% non-all-alpha target, and a pool at 0% is exactly what a NOT_RUN column would have hidden.
