...
**Nothing on a stock machine resolves `fold_class`, and the fallback that does is a different instrument.** The classifier reads secondary structure from `ss_codes` you pass it, a bare SS string, HELIX/SHEET records in the PDB, or `biotite.annotate_sse` — in that order. Measured on five real designs from three methods: generator PDBs carry **no HELIX/SHEET records at all**, so with no biotite every one came back `fold_class = NOT_RUN` and the ≥10% non-all-alpha evidence was simply absent.

**But biotite is P-SEA, and the frozen criterion is written in DSSP** — the kernel says so in both places (`_ss_from_biotite` is "P-SEA annotation via biotite"; the target is "not-all-alpha **under DSSP**"). The two assign helix fractions and strand runs by different rules, so a P-SEA-derived `fold_class` can disagree with the criterion the campaign is judged on. Treat it as an approximation and **say which method produced it**. The canonical path is the first one: run DSSP yourself and pass the codes in as `ss_codes`, which needs no extra dependency at all. Installing biotite is the convenient fallback, not the definition — with it, the same five structures came back `all_alpha`, i.e. a pool at 0% diversity, which is worth knowing and worth labelling as P-SEA rather than reporting as the target's own answer.
...
**Prefer a complex FASTA over hand-built settings — and read `tamarind-mcp-batch`'s rule precisely, because this looks like an exception to it and is not.** That skill says `fromFile` does not *construct* target-candidate complexes, and it is right: it will not join your target to your candidate for you, and a record holding the binder alone gets folded alone. What was measured here is the narrower fact that a record whose content is **already** the joined `TARGET:BINDER` value is submitted verbatim — the stored input comes back with both chains intact. So the join is still yours to build; `fromFile` only carries it. Write the FASTA programmatically from the generation table, upload it, and fan out with `fromFile` plus `sharedSettings`. That is the only route on which **no sequence passes through your hands**, which is the whole point: see the transcription rule below. Building explicit `settings` + matching `jobNames` is the route `tamarind-mcp-batch` documents for this case and works too, but every sequence in it is one you typed.

Two things `fromFile` does not give you: the record id does **not** become the job name (jobs come back numbered in file order), and nothing ties a returned row to a design. Reconcile by reading each job's stored input sequence back and matching it to the design's binder — never by position, and never by job name. **That reconciliation is only unique while the binder sequences are** — two methods can converge on the same binder, and then no amount of sequence matching says which design a job belongs to. Collapse identical binder sequences BEFORE submitting the scoring batch: fold each distinct sequence once and carry the set of design ids that share it, which is also the only way the per-method attribution on the sheet stays true. The selection kernel rejects exact-sequence duplicates anyway, so the second copy was never going to reach a panel.
