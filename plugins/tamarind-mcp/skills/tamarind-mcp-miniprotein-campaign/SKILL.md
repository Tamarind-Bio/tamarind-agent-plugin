...
**`fold_class` is reported under whichever secondary-structure method resolved it, and they are not interchangeable.** The classifier reads `ss_codes` you pass it, a bare SS string, HELIX/SHEET records in the PDB, or `biotite.annotate_sse` — in that order. The frozen fold-diversity target is defined **under DSSP**; biotite's fallback is **P-SEA**, a different assignment that can disagree on the very criterion the panel is judged against. So the canonical path is to run DSSP yourself and pass the codes per row as `ss_codes`, which needs no extra dependency; `campaign_gates.py` forwards them and stamps `fold_ss_method`. Where you fall back instead, **name the method in the report** rather than presenting the number as the target's own answer. Generator structures routinely carry no HELIX/SHEET records, so with neither codes nor biotite this column is NOT_RUN and the diversity evidence is absent — see [references/pool_schema.md](references/pool_schema.md).
