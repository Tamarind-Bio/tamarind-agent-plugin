...
**`proteina-complexa` publishes secondary-structure fractions, which settle the fold class ONE WAY.** Its per-design table declares `ss_alpha` and `ss_beta` — "fraction of binder residues in alpha-helix / beta-sheet secondary structure" — and it is the only roster entry that does. Non-all-alpha is *helix fraction below 0.70* **OR** *a run of at least 3 consecutive E/B residues* (`dssp_fold_class` in the kernel; `all_alpha` is its exact complement). So `ss_alpha < 0.70` settles **non-all-alpha on its own**, for free, on every row. `ss_alpha >= 0.70` settles nothing: an aggregate `ss_beta` cannot say whether those residues are CONSECUTIVE, and a design at 75% helix with scattered isolated strand residues is still `all_alpha`. Those rows still need residue-level `ss_codes`/DSSP. Use the columns for the half they decide and measure the rest — do not report a fold-diversity count derived from the two fractions alone.

It also declares `designability_scRMSD` (binder monomer refolded with ESMFold vs the generated structure) and `codesignability_scRMSD` (after a ProteinMPNN redesign). **These are designability evidence and are NOT the structural-plausibility gate.** That gate measures backbone geometry, steric clashes and core packing on the structure itself; scRMSD measures agreement with a refold, and a backbone can refold to the right global shape while carrying invalid peptide geometry or severe clashes. Record them as extra evidence; run the local plausibility check anyway.
