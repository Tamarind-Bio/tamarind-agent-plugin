...
- One exception to the subprocess rule: the Custom Tool lifecycle (create, upload, build, monitor, publish) has no `tamarind` subcommands, so `tamarind-custom-tool-deploy` drives it through the package's public `tamarind.Tamarind` SDK, which requires `tamarind-cli>=0.3.0`. That is still the vendor client, not a reimplementation - do not hand-roll HTTP, presigned upload, or polling for it. Running the finished tool stays on the CLI. Retire this exception if the CLI ever ships custom-tool subcommands.
